...
def apply_rotary_emb(
    ...
    pos = torch.arange(seqlen, device=device).unsqueeze(1)
    freq_range = torch.arange(0, head_dim, 2, device=device)
    freqs = pos * torch.exp(-freq_range * math.log(theta) / head_dim)
